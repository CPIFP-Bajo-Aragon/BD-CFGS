...
CREATE DATABASE IF NOT EXISTS ejercicio_embarques;
...
/* Obtener el peso total que ha enviado cada cliente a una ciudad distinta (mostrando el nombre de la ciudad)*/

select nomb_cli, ciu_nombre, sum(emb_peso) as total 
from cliente, embarque, ciudad where cliente.cli_id=embarque.emb_id_cli and ciudad.ciu_id=emb_destino
group by nomb_cli, ciu_nombre;
