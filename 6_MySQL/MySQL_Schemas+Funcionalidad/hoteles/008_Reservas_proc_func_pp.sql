...
begin
	...
	select idservicios into ser from servicios where nombreservicio=n_nombreservicio;
	update gastos set precio=precio*descuento where idservicios=ser and gastos.fecha between n_fechainicio and n_fechafin;
...
BEGIN
	...
	select ifnull(g.cantidad*g.precio,0) into valor_r from tipo_servicio ts, servicios s, gastos g
	where ts.nombreservicio=s.nombreservicio and s.idservicios=g.idservicios;
	
	/* calculo el teórico que esta en servicios*/
	select ifnull(g.cantidad*s.precio,0) into valor_t from tipo_servicio ts, servicios s, gastos g
	where ts.nombreservicio=s.nombreservicio and s.idservicios=g.idservicios;
