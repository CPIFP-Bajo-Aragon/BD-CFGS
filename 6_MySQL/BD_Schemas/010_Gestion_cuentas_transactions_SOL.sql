CREATE SCHEMA IF NOT EXISTS banco_proc_v3;
USE banco_proc_v3;
...
BEGIN
	...
	begin
        ...
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='ERROR EN LA CREACIÓN DE LA TABLA DE LOGS';  -- PERSONALIZADO Y PUEDO PONER EL SQLSTATE QUE QUIERA Y EL MENSAJE
	end;
	start transaction;
...
BEGIN
    ...
    commit;
	insert into LOG_SCRIPT values (0,'alta esquema','DISEÑO FÍSICO DE LA BD CREADO CORRECTAMENTE',current_timestamp());
END$$
DELIMITER ;
...
BEGIN
    ...
    commit;
	insert into LOG_SCRIPT values (0,'alta ejemplos','DATOS DE EJEMPLO DATOS DE ALTA CORRECTAMENTE',current_timestamp());
END$$
DELIMITER ;
...
declare exit HANDLER FOR sqlexception
	begin
		rollback;  
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='ERROR EN LA GERENACION DE LA BD';
	end;
start transaction;
CALL ALTA_LOG();
CALL ALTA_SCHEMA();
